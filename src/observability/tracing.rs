        ...
        if let Some(ref otlp) = self.otlp_exporter {
            ...
            let provider: TracerProvider = match otlp.exporter.as_str() {
                "otlp" => opentelemetry_otlp::new_pipeline()
                    .tracing()
                    .with_exporter(opentelemetry_otlp::new_exporter().tonic().with_endpoint(&otlp.endpoint))
...
mod tests {
    ...
    fn test_tracing_builder_with_level() {
        let result = TracingBuilder::new().with_fmt().with_level("dbnexus=debug").init();
        assert!(result.is_ok() || result.is_err());
    }

    #[test]
    fn test_tracing_builder_json() {
        let result = TracingBuilder::new().with_json().with_level("info").init();
        assert!(result.is_ok() || result.is_err());
    }
