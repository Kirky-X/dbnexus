// Copyright (c) 2026 Kirky.X
// SPDX-License-Identifier: MIT
//! Neo4j 图数据库连接（stub，T029 将填充真实实现）
//!
//! 当前为占位类型，所有 [`GraphConnection`] 方法返回明确错误。
//! T029 将替换为基于 `Arc<neo4rs::Graph>` 的真实实现。

use crate::database::graph::{GraphConnection, GraphExecResult, GraphTransaction};
use crate::foundation::{DbError, DbResult};
use async_trait::async_trait;

/// Neo4j 图数据库连接（stub）
///
/// 这是 T024 创建的占位类型，T029 将替换为真实实现
/// （包装 `Arc<neo4rs::Graph>`）。
#[derive(Debug, Clone)]
pub struct Neo4jConnection {
    _private: (),
}

impl Neo4jConnection {
    /// 创建占位连接（仅用于 T024 的 DbConnection 接线测试）
    ///
    /// T029 将实现真实的 `new(url, user, password) -> DbResult<Self>`。
    #[cfg(test)]
    pub(crate) fn new_placeholder() -> Self {
        Self { _private: () }
    }
}

#[async_trait]
impl GraphConnection for Neo4jConnection {
    async fn execute_cypher(&self, _cypher: &str) -> DbResult<GraphExecResult> {
        Err(DbError::Config(
            "Neo4jConnection not yet implemented (T029 will implement)".to_string(),
        ))
    }

    async fn health_check(&self) -> DbResult<()> {
        Err(DbError::Config(
            "Neo4jConnection not yet implemented (T029 will implement)".to_string(),
        ))
    }

    async fn begin_graph_txn(&self) -> DbResult<Box<dyn GraphTransaction + Send>> {
        Err(DbError::Config(
            "Neo4jConnection not yet implemented (T029 will implement)".to_string(),
        ))
    }

    fn backend_name(&self) -> &'static str {
        "neo4j"
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[tokio::test]
    async fn test_neo4j_stub_execute_cypher_returns_error() {
        let conn = Neo4jConnection::new_placeholder();
        let result = conn.execute_cypher("RETURN 1").await;
        assert!(result.is_err(), "stub should return error");
    }

    #[tokio::test]
    async fn test_neo4j_stub_health_check_returns_error() {
        let conn = Neo4jConnection::new_placeholder();
        let result = conn.health_check().await;
        assert!(result.is_err(), "stub should return error");
    }

    #[tokio::test]
    async fn test_neo4j_stub_begin_graph_txn_returns_error() {
        let conn = Neo4jConnection::new_placeholder();
        let result = conn.begin_graph_txn().await;
        assert!(result.is_err(), "stub should return error");
    }

    #[test]
    fn test_neo4j_backend_name() {
        let conn = Neo4jConnection::new_placeholder();
        assert_eq!(conn.backend_name(), "neo4j");
    }
}
