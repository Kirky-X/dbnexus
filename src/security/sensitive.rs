...
impl SensitiveMasker {
    ...
    pub fn mask(data: &str, mask_type: MaskType) -> SensitiveResult<String> {
        ...
        match mask_type {
            ...
            MaskType::Custom { keep_prefix, keep_suffix } => {
    ...
    fn mask_id_card(id: &str) -> SensitiveResult<String> {
        // 移除可能的空格和横线
        let cleaned: String = id.chars().filter(|c| c.is_ascii_digit() || *c == 'X' || *c == 'x').collect();
...
mod tests {
    ...
    fn test_mask_custom() {
        // 自定义保留前后位数
        assert_eq!(
            SensitiveMasker::mask("1234567890", MaskType::Custom {
                keep_prefix: 2,
                keep_suffix: 2
            }).unwrap(),
            "12******90"
        ...
        // 保留位数超过总长度
        assert_eq!(
            SensitiveMasker::mask("123", MaskType::Custom {
                keep_prefix: 2,
                keep_suffix: 2
            }).unwrap(),
            "123"
