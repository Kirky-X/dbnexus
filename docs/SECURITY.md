...
本文档描述 Dbnexus 的安全支持策略、漏洞报告流程、安全设计概览与使用时的安全最佳实践。安全架构的完整设计细节（含纵深防御图与权限模型）见[架构文档](ARCHITECTURE.md)。
...
| 版本线 | 状态 | 说明 |
|--------|------|------|
| 0.6.x（当前 `0.6.0-rc.3`） | ✅ 支持中 | 安全修复随补丁版本发布 |
...
相关文档：[📖 用户指南](USER_GUIDE.md) ｜ [📘 API 参考](API_REFERENCE.md) ｜ [🏗️ 架构文档](ARCHITECTURE.md)
